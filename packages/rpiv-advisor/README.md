...
| `effort` | Reasoning effort for the reviewer: `minimal`, `low`, `medium`, `high`, `xhigh`. Written by `/advisor`; only levels supported by the selected model are offered. | absent — no reasoning sent |
