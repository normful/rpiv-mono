...
Model and reasoning effort are separate axes, and the second one is easy to under-use. There are five graded levels (`minimal`, `low`, `medium`, `high`, `xhigh`) plus a real `off`; `/rpiv-models` offers only the levels supported by the selected model. The distinction that trips people up:
